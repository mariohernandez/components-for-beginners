# Training resources

### Assets

[Slide deck and other training material](https://drive.google.com/drive/folders/1-gIa-bYV1LXhK6t51CWarrX-h1DUvzbO?usp=sharing)

## Twig

* [Twig documentation](https://twig.symfony.com/doc/2.x/templates.html)

### CSS/Sass

* [Font sizing with REMs](https://snook.ca/archives/html_and_css/font-size-with-rem)
* [BEM 101](https://css-tricks.com/bem-101/)
* [BEM Cheat Sheet](https://9elements.com/bem-cheat-sheet/)
* [BEM Resources](https://en.bem.info/)
* [Sass basics](https://www.sass-lang.com/guide/)
* [Using Sass Partials](https://dev.to/sarah_chima/using-sass-partials-7mh)
* [Name that color](http://chir.ag/projects/name-that-color/#6195ED) \(color naming tool\)

### Front End

* [Pattern Lab](https://patternlab.io/)
* [Learn to code HTML & CSS](https://learn.shayhowe.com/)

### Command Line

* [10 Basic commands every web designer should grasp](https://www.hongkiat.com/blog/web-designers-essential-command-lines/)

### Coding

1. [VS Code User and Workspace Settings](https://code.visualstudio.com/docs/getstarted/settings)
2. [JSON Formatter and Validator](https://jsonformatter.curiousconcept.com/)

